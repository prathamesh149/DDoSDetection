...
public class KNN {
	...
	public int magic()
	{
		...
		{	// find the knn
	        ...
	        {
	            double dist = trainingSamples.get(i).distance(testSamples.get(j));
	            if (i<knn)
	            {
	                ...
	                if (dist < biggestd)
	                   {
	                		...
	                		distances[biggestindex].label = trainingSamples.get(i).getPacketType();
	                   }
	            }
	        }
...
//					 classification = Math.random() < 0.5; // if tie, randomly break it
			 ...
			 //System.out.println(classification+" "+ testSamples.get(j).getPacketType());
				 // count classification errors
			 if (!classification.equals(testSamples.get(j).getPacketType()))
			 {
				 System.out.println(error+" out of "+j+" "+testSamples.size()+" classified incorrectly.");
				 error ++;
			 }
		}
		System.out.println(error+" out of "+testSamples.size()+" classified incorrectly.");
		return error;
	}
